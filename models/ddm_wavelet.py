...
class DenoisingDiffusion_Wavelet(object):
    def __init__(self, args, config):
        ...
        if os.path.isfile(self.args.resume):
            # Only check rank if distributed is initialized
            if dist.is_initialized():
                if dist.get_rank() >= 0:
                    self.load_ddm_ckpt(self.args.resume)
            else:
                # Single GPU/CPU mode - always load checkpoint
                self.load_ddm_ckpt(self.args.resume)

        # Only wrap with DistributedDataParallel if distributed is initialized
        if dist.is_initialized():
    ...
    def load_ddm_ckpt(self, load_path, ema=False):
        ...
        filtered_state_dict = {
            ...
            if not (k.startswith('rev_down_module_lists') or k.startswith('rev_up_module_lists'))
        }
        skipped_keys = [k for k in state_dict.keys() if k not in filtered_state_dict]
        if skipped_keys:
            print(f"Skipping {len(skipped_keys)} reversible connector weights that are not used by the current UNet implementation.")
        ...
        try:
            ...
            print(f"Optimizer state could not be restored ({err}); using freshly initialized optimizer params instead.")

        ema_state = checkpoint['ema_helper']
        if isinstance(ema_state, dict):
            ema_filtered = {
                k: v for k, v in ema_state.items()
                if not (k.startswith('rev_down_module_lists') or k.startswith('rev_up_module_lists'))
            }
            skipped_ema = len(ema_state) - len(ema_filtered)
            if skipped_ema:
                print(f"Skipping {skipped_ema} reversible EMA entries that are not needed.")
            ema_state = ema_filtered
    ...
    def train(self, DATASET):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y, total) in enumerate(train_loader):
                ...
                if (world_size>1 and self.step % self.config.training.validation_freq == 0) or (world_size==1 and self.step % 10 == 0):
                    if rank == 0:
                        self.model.eval()
                ...
                if self.step % self.config.training.snapshot_freq == 0 or self.step == 1:
                    rank = dist.get_rank() if dist.is_initialized() else 0
                    if rank == 0:
                        # Get the underlying model (unwrap if DDP)
